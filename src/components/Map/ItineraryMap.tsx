...
function loadAmapAPI(): Promise<void> {
  ...
  amapLoadPromise = new Promise((resolve, reject) => {
    ...
    if (!key) {
      reject(
        new Error(
          "PUBLIC_AMAP_KEY not configured. Get a key at https://console.amap.com/dev/key/app",
        ),
      );
      return;
    }
  ...
  const initMap = useCallback(async () => {
    ...
    try {
          ...
          ((m: unknown, content: string) => {
            (m as { on: (event: string, handler: () => void) => void }).on("click", () => {
              ...
              (infoWindow as { open: (map: unknown, position: unknown) => void }).open(
                map,
                (
                  m as { getPosition: () => { getLng: () => number; getLat: () => number } }
                ).getPosition(),
              );
            });
          })(marker, infoContent);
      ...
      if (allPoints.length > 1) {
        ...
        (
          map as {
            setFitView: (overlays?: unknown, immediately?: boolean, margin?: number[]) => void;
          }
        ).setFitView(undefined, false, [50, 50, 50, 50]);
      }
    } catch (e) {
