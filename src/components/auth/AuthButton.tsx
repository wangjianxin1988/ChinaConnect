...
export function AuthButton({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      try {
        ...
        setUser({
          ...
          displayName: profile?.display_name || currentUser.email?.split("@")[0] || "User",
    ...
    const {
      data: { subscription },
    } = onAuthStateChange(async (event, authUser) => {
  ...
  // Loading state
  if (isLoading) {
    return <div className={cn("w-9 h-9 rounded-full bg-gray-200 animate-pulse", className)} />;
  }
