...
# 1) add CJK price-field collection after the main actionable loop (before the closing of the for-fn loop)
old = """  console.log(`[${lang}] actionable fields: ${actionable.length}`);
  totalFields += actionable.length;"""
new = """  // Additional pass: CJK-containing price fields (priceRange/ticketPrice) where
  // EN source is clean English and ja is Japanese -> translate EN source.
  const PRICE_LEAF = new Set(["priceRange", "ticketPrice"]);
  const extraActionable = [];
  for (const fn of fs.readdirSync(langDir).filter((f) => f.endsWith(".json"))) {
    ...
    for (const [p, v] of Object.entries(walk(target))) {
      ...
      extraActionable.push({ file: fn, path: p, value: v, enValue: enV, jaValue: jaV });
    }
  }
  for (const a of extraActionable) {
    if (!actionable.some((x) => x.file === a.file && x.path === a.path)) actionable.push(a);
  }
