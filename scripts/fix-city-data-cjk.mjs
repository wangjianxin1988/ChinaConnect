// Fix mixed Chinese residue in cities-i18n data for non-zh languages.
// For each string field containing CJK in the target language file:
//   - if the ja counterpart is a pure CJK proper noun -> keep (baseline-safe)
//   - otherwise re-translate the field from the ja value into the target language.
// Usage: node scripts/fix-city-data-cjk.mjs [--lang=ko] [--dry-run]
...
// A proper noun: CJK + punctuation + digits, no kana, no latin.
function isPureProperNoun(s) {
  if (!CJK_RE.test(s)) return false;
  if (KANA_RE.test(s)) return false;
...
function unescapeJson(s) { return s.replace(/\\"/g, '"').replace(/\\\\/g, "\\").replace(/\\n/g, "\n"); }
...
Source strings are Japanese translations of Chinese travel content (city guides, attraction tips, tickets, addresses).
Input JSON:
${body}
RULES:
...
- Keep proper nouns (place names, attraction names, dish names) in their standard form with a ${TARGETS[lang]} gloss where helpful.
- Keep numbers, prices, times, units, phone numbers and URLs unchanged.
- Output must be fluent natural ${TARGETS[lang]} with NO Chinese characters.`;
}
...
async function translateBatch(batch, lang) {
  ...
  while (attempt < RETRY_ATTEMPTS && remaining.length > 0) {
    ...
    try {
      ...
      remaining.forEach((s, i) => {
        ...
        if (typeof raw === "string" && raw.length > 0) { resultMap.set(s, raw); accepted += 1; }
        else newRemaining.push(s);
      });
      if (newRemaining.length < remaining.length) console.warn(`  partial: +${accepted} accepted, ${newRemaining.length} remaining`);
  ...
  for (const s of remaining) {
    try {
      ...
      const result = extractJson(content);
      if (typeof result?.k0 === "string" && result.k0.length > 0) resultMap.set(s, result.k0);
    } catch { /* keep */ }
...
for (const lang of LANGS) {
  ...
  const actionable = []; // { file, path, value, jaValue }
  for (const fn of fs.readdirSync(langDir).filter((f) => f.endsWith(".json"))) {
    ...
    for (const [p, v] of Object.entries(walk(target))) {
      ...
      // ja is a pure CJK proper noun -> keep as-is (baseline-safe)
      if (isPureProperNoun(jaV)) continue;
  ...
  for (const a of actionable) {
    ...
    if (!newVal || newVal === a.jaValue) continue;
