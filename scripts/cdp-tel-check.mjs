...
for (const t of TARGETS) {
  ...
  const stats = await page.evaluate(() => {
    ...
    const sources = Array.from(document.querySelectorAll("a"))
      .filter(
        (a) =>
          /dianping|tripadvisor|xiaohongshu|meituan|michelin/i.test(a.href) ||
          /sources|reference|数据源/i.test(a.textContent),
      )
      .slice(0, 5)
      .map((a) => ({ href: a.href, text: a.textContent.trim().slice(0, 40) }));
    return {
      telCount: tel.length,
      telSamples: Array.from(tel)
        .slice(0, 3)
        .map((a) => a.href),
      mapCount: map.length,
      sources,
