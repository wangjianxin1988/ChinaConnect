...
export default function LanguageSwitcher({ currentLang, className = '' }: LanguageSwitcherProps) {
  ...
  const handleSwitch = (lang: Language) => {
    ...
    const pathWithoutLang = currentPath.replace(/^\/(en|zh-CN|zh-TW|ja|ko|th|vi|ru|fr|de|ar|fa)\//, '/');
              ...
              <li key={lang.code}>
                ...
                >
                  <span className="flex items-center gap-2">
                    <span className="text-base">{lang.code === 'zh-CN' ? '简' : lang.code === 'zh-TW' ? '繁' : lang.code === 'en' ? 'EN' : lang.code.toUpperCase()}</span>
