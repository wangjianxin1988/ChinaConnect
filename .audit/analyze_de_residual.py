...
lang = 'de'
all_target = {fn: json.load(io.open(os.path.join(base, fn), encoding='utf-8')) for fn in os.listdir(base) if fn.endswith('.json')}
rows = []
for fn, target in all_target.items():
    slug = fn[:-5]
...
long_rows = [r for r in rows if r[0] >= 5]
print('total residual:', len(rows), '| >=5 words:', len(long_rows), '| 3-4 words:', len([r for r in rows if 3 <= r[0] < 5]))
print()
for n, p, v in sorted(long_rows, key=lambda r: -r[0])[:60]:
    print('%d %s = %s' % (n, p, v[:110]))
