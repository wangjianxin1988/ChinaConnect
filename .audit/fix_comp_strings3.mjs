...
const fixlist = JSON.parse(fs.readFileSync(".audit/_comp_fixlist3.json", "utf8"));
...
function valid(v, lang, src) {
  ...
  const CJK = /[\u3400-\u9fff]/;
  const KANA = /[\u3040-\u30ff]/;
  if (lang === "zh-CN" || lang === "zh-TW") { if (!CJK.test(v)) return false; } else { if (CJK.test(v) || KANA.test(v)) return false; }
  if (lang === "ar" || lang === "fa") {
    const AR = /[\u0600-\u06ff]/;
...
fs.writeFileSync(".audit/_comp_fix_results3.json", JSON.stringify(results, null, 1), "utf8");
