...
fixes = {
    ...
    "auth": {"myAccount": "My Account", "profileCenter": "Profile Center", "signIn": "Sign In", "signOut": "Sign Out"},
    ...
    "errors": {"generic": "Something went wrong", "network": "Network error", "unauthorized": "Unauthorized"},
    ...
    "language": {"code": "en", "dir": "ltr", "name": "English", "nativeName": "English"},
    "nav": {"attractions": "Attractions", "selectLanguage": "Select Language"},
    "notFound": {
    ...
    "recents": {"clearAll": "Clear all", "empty": "No recent visits", "title": "Recent visits"},
    "register": {"emailLabel": "Email address", "passwordLabel": "Password", "title": "Register"},
    "tooltips": {"close": "Close", "more": "More", "open": "Open"},
}
...
for section, kv in fixes.items():
    for key, val in kv.items():
        ...
        en_region, n = pat.subn(lambda m: m.group(1) + m.group(2) + m.group(3) + '"' + newval + '"' + m.group(4), en_region)
        count += n

result = text[:en_start] + en_region + text[ja_start:]
out_bytes = result.encode("utf-8-sig") if raw.startswith(b"\xef\xbb\xbf") else result.encode("utf-8")
with open(path, "wb") as f:
    f.write(out_bytes)
print("EN replaced:", count)
print("CRLF:", result.count("\r\n"), "LF-only:", result.count("\n") - result.count("\r\n"))
