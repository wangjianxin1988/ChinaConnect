...
for (const lang of langs) {
  ...
  if (langStart < 0) { console.log('NOT FOUND lang:', lang); continue; }
  ...
  for (; i < out.length; i++) {
    if (out[i] === '{') depth++;
    else if (out[i] === '}') { depth--; if (depth === 0) { recentsEnd = i; break; } }
  }
  if (recentsEnd < 0) continue;
...
console.log('Inserted features for', inserted, 'langs');
