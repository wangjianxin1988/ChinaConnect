...
for (const t of TARGETS) {
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      const cult = !!document.querySelector(
        '[class*="cultural-trigger" i], button[class*="culture"][class*="fixed" i]',
      );
      const floatingCult = Array.from(document.querySelectorAll("button"))
        .filter(
          (b) =>
            /culture|\u6587\u5316/.test(b.textContent) &&
    ...
    await page.screenshot({
      path: "screenshots/wave0" + t.replace(/\//g, "_") + ".png",
      fullPage: false,
    });
