...
async function getCityClimate(en) {
  try {
    const geoRes = await fetch(
      `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(en)}&count=1&language=en`,
    );
    ...
    const wxRes = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=auto&forecast_days=7`,
    );
...
function adjustTourism(base, climate) {
  ...
  const tempFactor =
    climate.avgTemp > 0 ? Math.max(0.7, 1.0 - Math.abs(climate.avgTemp - 22) * 0.02) : 1.0;
...
function scoreLivability(climate) {
  ...
  const tempScore =
    climate.avgTemp > 0 ? Math.max(30, 100 - Math.abs(climate.avgTemp - 20) * 2.5) : 60;
...
async function main() {
  ...
  for (let i = 0; i < CITIES.length; i += 5) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      console.log(
        `  ${tier} ${composite.toFixed(1).padStart(5)} ${city.zh.padEnd(5)} E:${economy.toFixed(0)} I:${international.toFixed(0)} T:${tourism.toFixed(0)} L:${livability.toFixed(0)} | ${climate.avgTemp.toFixed(1)}°C ${climate.precip.toFixed(1)}mm`,
      );
    }
    ...
    .map((r) => ({
      ...
      score_breakdown: [
        ...
        { source: "baseline_caa", metric: "international", value: r.internationalScore },
        {
          source: "openmeteo",
          metric: "tourism",
          value: r.tourismScore,
          temp: r.climate.avgTemp,
          precip: r.climate.precip,
        },
        {
          source: "openmeteo",
          metric: "livability",
          value: r.livabilityScore,
          population: r.climate.population,
        },
      ],
  ...
  for (const r of results) {
    tiers[r.tier]++;
    console.log(
      `  ${r.tier} ${r.compositeScore.toFixed(1).padStart(5)} #${String(r.rank).padStart(2)} ${r.slug.padEnd(12)} E:${r.economyScore.toFixed(0).padStart(3)} I:${r.internationalScore.toFixed(0).padStart(3)} T:${r.tourismScore.toFixed(0).padStart(3)} L:${r.livabilityScore.toFixed(0).padStart(3)}`,
    );
  }
