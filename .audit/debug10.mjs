...
const en = dicts.en, ja = dicts.ja, ko = dicts.ko;
for (const k of ["scenicSpots.title", "scenicSpots.spotsCount"]) {
  const v = ko.get(k)?.value;
  const enV = en.get(k)?.value;
  const jaV = ja.get(k)?.value;
  const isProse = (s) => s.includes(" ") || s.length > 12;
  console.log(JSON.stringify({ k, v, enV, jaV, prose: isProse(enV || ""), keep: isKeepableToken(enV || "") }));
}
