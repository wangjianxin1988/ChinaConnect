  ...
  return {
    ...
    breakdown: [
      { source: "yicai", weight: 0.3, rawScore: economy, weightedContribution: economy * 0.3 },
      {
        source: "gawc",
        weight: 0.25,
        rawScore: international,
        weightedContribution: international * 0.25,
      },
      { source: "ctrip", weight: 0.15, rawScore: tourism, weightedContribution: tourism * 0.15 },
      {
        source: "gaode",
        weight: 0.1,
        rawScore: GAODE_SCORES[slug] || 20,
        weightedContribution: (GAODE_SCORES[slug] || 20) * 0.1,
      },
      {
        source: "dazhong",
        weight: 0.2,
        rawScore: DAZHONG_SCORES[slug] || 20,
        weightedContribution: (DAZHONG_SCORES[slug] || 20) * 0.2,
      },
    ],
  };
}
...
async function main() {
  ...
  for (const rec of scoreRecords) {
    tiers[rec.tier]++;
    console.log(
      `  ${rec.tier} ${rec.composite_score.toFixed(1).padStart(5)} #${String(rec.overall_rank).padStart(2)} ${ALL_CITIES.find((c) => rec.city_id === cityIdMap[c.slug])?.name || rec.city_id}`,
    );
  }
  console.log(
