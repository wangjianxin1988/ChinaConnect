...
const KANA = /[ぁ-んァ-ヶ]/;
const CN = new RegExp("[广门车场机东乐园馆岛图发汉阳长马鱼龙兴华风观远达丽质铁这还现县乡为务报卖单难态说认让议变盘环实贵惠味正宗推酱面店豆汁炒肝爆肚涮羊肉饭馆务员菜单营打折优导航里走骑坐推荐实惠味道本地人营业时间开放时间门票免费收费套餐预约订位评价好评]");
const count = new Map(); const loc = new Map();
for(const f of fs.readdirSync(dir).filter(x=>x.endsWith(".json"))){
  const d = JSON.parse(fs.readFileSync(path.join(dir,f),"utf8"));
  (function walk(o,p){
    if(typeof o==="string"){
      const isProse = /\.(description|tips)(\[\d+\])?$/.test(p) || /highlights\[\d+\]$/.test(p) || /culturalTips\[\d+\]\.(title|content)$/.test(p);
      if(o.length>=8 && !KANA.test(o) && isProse && CN.test(o)){
...
console.log("distinct CN-flavored prose:", arr.length, "occ:", arr.reduce((s,[,c])=>s+c,0));
