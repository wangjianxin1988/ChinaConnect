...
for (const t of TARGETS) {
  ...
  try {
    ...
    await page.evaluate(() => { localStorage.removeItem('chinaconnect_lang'); localStorage.removeItem('chinaconnect_locale'); });
    const data = await page.evaluate(() => {
      ...
      const floatingCult = Array.from(document.querySelectorAll('button')).filter(b => /culture|cultural/i.test(b.textContent) && b.getBoundingClientRect().bottom > window.innerHeight - 250).map(b => b.textContent.trim().slice(0,30));
      ...
      return { aiLabel, h1: h1.trim().slice(0,80), cult, floatingCult, realImgs, tel, map, aiPageH1 };
    });
    await page.screenshot({ path: 'screenshots/wave0-v2' + t.replace(/\//g,'_') + '.png', fullPage: false });
