...
lines = text.split("\n")  # keep CRLF intact via \r in line ends? no - split keeps \r at line end

# Work on the raw text with line-anchored replacements inside ja/ko blocks only.
def block_lines(src, lang):
    start_m = re.search(r'^  "' + lang + r'": \{|^  ' + lang + r': \{', src, re.M)
    if not start_m:
        return None, None
    start = start_m.start()
    # next top-level lang key
    next_m = re.search(r'^  "?(?:[a-z]{2}(?:-[A-Z]{2})?)"?:\s*\{', src[start + 1:], re.M)
    end = start + 1 + (next_m.start() if next_m else len(src) - start - 1)
    return start, end

def patch_in_range(src, start, end, mapping):
...
for lang, mapping in [("ja", {"code": "ja", "dir": "ltr", "name": "Japanese", "nativeName": "\u65e5\u672c\u8a9e"}),
                      ("ko", {"code": "ko", "dir": "ltr"})]:
    start, end = block_lines(out, lang)
    if start is None:
        print(lang, "block not found!")
        continue
    region, n = patch_in_range(out[start:end], 0, len(out[start:end]), mapping)
    # patch_in_range is relative; simpler: apply on substring
    out = out[:start] + region + out[end:]
    print(f"{lang}: replaced {n}")
